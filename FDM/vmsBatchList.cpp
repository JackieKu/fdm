...
bool vmsBatchList::Create(LPCTSTR pszTemplate, LPCTSTR pszNumbers, TCHAR chStart, TCHAR chEnd)
{
	m_v.clear ();
...
#define check_nz if (*pszNumbers == 0) return false;
		...
		if (*pszNumbers == 0 || *pszNumbers == _T(',')) {
			...
			if (*pszNumbers == _T('-'))
			{
				...
				nEnd = _ttoi (pszNumbers);
				skip_number;
			}

			while (*pszNumbers && *pszNumbers != _T(',')) {

				TCHAR chWhat = *pszNumbers++;
				skip_spaces;

				int nValue = _ttoi (pszNumbers);
				skip_number;
...
bool vmsBatchList::CreateAZBatch(LPCTSTR pszTemplate, TCHAR chStart, TCHAR chEnd)
{
	for (TCHAR c = chStart; c <= chEnd; c++)
	{
		CString str = pszTemplate;
		CString strA = c;
		CString stra = c;
