...
void CSchedule_WTSPage::BuildDownloadsList()
{
	...
	for (int i = 0; i < cDlds; i++)
	{
		...
		if (bToUnsel)
		{
			m_wndUnselDlds.InsertItem (cUnSel, dld->pMgr->get_URL (), 0);
			m_wndUnselDlds.SetItemData (cUnSel++, dld->nID);
		}
		else
		{
			m_wndSelDlds.InsertItem (cSel, dld->pMgr->get_URL (), 0);
			m_wndSelDlds.SetItemData (cSel++, dld->nID);
		}
	}
...
void CSchedule_WTSPage::ApplyLanguage()
{
	...
	CString str = LS (L_CHANGETUMTO); str += _T(':');
