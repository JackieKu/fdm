...
DWORD WINAPI CDlg_CreateNewTorrent::_threadCreateNewTorrent(LPVOID lp)
{
	...
	*psz = 0;
	...
	pBtFile->CreateNewTorrent (strSrcFile, pszTrackers, NULL, NULL, strComment, 
	...
	else
	{
		...
		if (dlg.DoModal () == IDOK)
		{
			...
			if (bOk)
			{
				if (pthis->IsDlgButtonChecked (IDC_SEED) == BST_CHECKED)
				{
					...
					while (strSavePath.IsEmpty () == FALSE && strSavePath.Right (1) != _T('\\') &&
							strSavePath.Right (1) != _T('/'))
		...
		pthis->EndDialog (bOk ? IDOK : IDCANCEL);
	}
